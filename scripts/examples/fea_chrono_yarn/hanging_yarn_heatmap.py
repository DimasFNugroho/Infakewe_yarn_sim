"""One-end-fixed hanging yarn with stretch heatmap visualization.

This scenario keeps the start node fixed and lets the other end fall freely.
An FEA scalar overlay is enabled to color the cable by axial strain so regions
under stronger stretch are visually highlighted.
"""
...
PRECHECK_DURATION_S = 0.1
PHYSICS_STEPS_PER_RENDER = 2
PRINT_INTERVAL_S = 0.5
NODE_DRAG_GAMMA_S_INV = 0.25

# Nomex 2670 dtex / Z90 preset:
# - 2670 dtex -> linear mass density = 0.267 g/m
# - Nomex specific gravity ~1.38 (rho ~1380 kg/m^3)
# - Twisted yarns have voids; use packing factor to estimate effective diameter.
...
# Filament modulus is high; for twisted yarn (Z90) we use an effective value.
...
DEFAULT_CONFIG_SECTION = "heatmap"
DEFAULT_CFG = {
    "simulation": {
        "dt_s": 5e-4,
    ...
    "visualization": {
        ...
        "window_title": "Hanging Yarn Stretch Heatmap (ANCF)",
...
def run_headless_precheck(
    ...
    *,
    ...
    seconds: float,
    drag_gamma_s_inv: float,
) -> bool:
    if seconds <= 0.0:
        return True
    for i in range(max(1, int(seconds / dt))):
        apply_node_drag(cable, drag_gamma_s_inv)
        step_system_with_substeps(system, dt, integration_substeps)
...
def initialize_fall_state(
    ...
    *,
    ...
    """Apply tiny geometric perturbation to avoid perfect symmetry lock."""
    n = len(cable.nodes)
    if n <= 1:
        return
    for i, node in enumerate(cable.nodes):
        if i == 0:
            continue  # keep the fixed endpoint exactly at its anchor location
        w = i / float(n - 1)
        p = node.GetPos()
        # Smooth initial sag with maximum at the free side.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Hanging yarn heatmap simulation")
    parser.add_argument(
        "--config",
        type=Path,
...
def main() -> None:
    ...
    yarn_cfg = FEAHangingYarnConfig(
        length=float(yarn_c["length_m"]),  # rest length used by the cable builder
    ...
    print("=== hanging_yarn_heatmap.py parameters ===")
    ...
    print(
        ...
        f"radius={yarn_cfg.radius} m E={yarn_cfg.young_modulus:.2e} Pa"
    )
    print(
    ...
    # Axial strain heatmap: compressed (cool) to stretched (warm).
    attach_fea_strain_overlay(
        scene.mesh,
    ...
    if not run_headless_precheck(
        scene.system,
        scene.cable,
        dt=sim_cfg.dt,
        integration_substeps=int(sim_c.get("integration_substeps", 1)),
        seconds=float(rt_c["precheck_duration_s"]),
        drag_gamma_s_inv=float(damp_c["node_drag_gamma_s_inv"]),
    ):
        print("Reduce dt or young_modulus; increase damping for stability.")
        return
    ...
    startup_delay_frames = int(startup_delay_s / max(1e-12, dt))
    frame_idx = 0
